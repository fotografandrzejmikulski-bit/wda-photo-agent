...
export async function POST() {
  ...
  const scope = process.env.ADOBE_SCOPE || "openid,AdobeID";
  ...
  if (!response.ok) {
    return NextResponse.json({ error: "Autoryzacja Adobe nie powiodła się.", details: payload }, { status: response.status });
  }
